    ...
    <ScreenContainer className="px-5 pt-3" containerClassName="bg-background" edges={["top", "left", "right", "bottom"]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={[styles.previewFrame, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <View style={[styles.previewTag, { backgroundColor: "rgba(0,0,0,0.58)" }]}><IconSymbol name="sparkles" size={13} color="#fff" /><Text style={styles.previewTagText}>{outputUrl ? "Offline rendered" : "Natural preview"}</Text></View>
        </View>
        ...
        <View style={[styles.prototypeNote, { backgroundColor: `${colors.warning}12`, borderColor: `${colors.warning}35` }]}>
          <Text style={[styles.prototypeNoteText, { color: colors.muted }]}>{outputUrl ? "This video was rendered locally on your device with the Free Offline Edition. Choose a social app below to share it." : hasVideo ? "Choose a social app below to pass the local video into its composer." : params.audioUri ? "Image preview keeps the source photo visible while the selected audio and sync timing are active. Start a new render to generate the animated video." : "Add an audio reference to activate image-to-lip-sync preview."}</Text>
        </View>
