...
describe("lip-sync provider errors", () => {
  it("detects Sync Labs insufficient-credit responses", () => {
    const error = new Error(
      'Sync Labs request failed (402): {"title":"Insufficient credit","status":402}',
    );

    expect(isProviderCreditError(error)).toBe(true);
    expect(friendlyLipSyncError(error)).toContain("Sync Labs");
  });
